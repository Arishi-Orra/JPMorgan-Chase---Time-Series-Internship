...
def train_model(num_steps=100000, batch_size=512, lr=1e-3, eval_every=500):
    # Initialize networks
    policy_net = PolicyNet()                # maps (log k, b, log z) --> (dlog k, b')
    value_net  = ValueNet()                 # maps (log k, b, log z) --> V(k, b, z)

    optimizer = tf.keras.optimizers.Adam(lr)    # Optimizer shared by both networks
    ...
    for step in range(1, num_steps + 1):
        ...
        logk, b, logz, eps1, eps2 = sample_batch(batch_size)
        # ---- Compute AiO loss and its gradients ----
        with tf.GradientTape() as tape:
            loss = bellman_aio_loss(policy_net, value_net, logk, b, logz, eps1, eps2)
